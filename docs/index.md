...
| [development/state-transition-model.md](development/state-transition-model.md) | Contributors changing anything that touches a session's lifecycle | Every lifecycle mutation, its initiator, its precondition, its durable and cache writes, its write ordering, and its retry and terminal behaviour — plus the explicit transition table, the transitions the code performs that the model forbids, and what the central boundary must own. |
