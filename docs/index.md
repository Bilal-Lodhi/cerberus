...
| [development/idempotency-model.md](development/idempotency-model.md) | Contributors changing a paid route, maintainers assessing cost exposure | The two paid routes and their measured exposure to a retry after a lost response, the decision to re-accept that risk for the multi-writer phase, the claim protocol that would close it (one collection, a unique index, a TTL index), the exact pending-operation and process-death semantics, and the eight assertions that would prove it. Superseded as a *decision* by the state model below; kept as the record of why the exposure was accepted then. |
| [development/paid-operation-state-model.md](development/paid-operation-state-model.md) | Contributors changing a paid route, operators reasoning about a retry | The state machine for the two routes that spend money: what each step does and what it leaves durable, the corrected paid-call count per route, the `Idempotency-Key` contract, the canonical request fingerprint, the atomic claim and its unique index, replay and conflict semantics, the pending lease and stale reclaim, every failure path and crash window including the one that cannot be closed, retention, the redaction rules, the query cost, and an implementation-status table that is the single source of truth for what is done. |
| [development/multi-writer-checkpoint.md](development/multi-writer-checkpoint.md) | Contributors, maintainers, anyone assessing the project | The record of the **multi-writer consistency and trust-boundary** cycle: the eight merged pull requests, the five defects and how each was found, the exit criteria with their state, the verification at the checkpoint, the immutability proof for the four published tags, and the eight limitations accepted rather than fixed. |
| [development/paid-operation-idempotency-checkpoint.md](development/paid-operation-idempotency-checkpoint.md) | Contributors, maintainers, anyone assessing the project | The record of the **durable idempotency and side-effect safety** cycle: the eight merged pull requests, the seven defects and how each was found, the design in one page, the exit criteria with their state, the verification at the checkpoint, the immutability proof for the five published tags, and the ten limitations accepted rather than fixed. |
...
| [operations/multi-replica.md](operations/multi-replica.md) | Operators running more than one API process | What is safe with N replicas and why, what multiplies or does not work at all (rate limiting, per-process identity handles, notification duplication, the non-idempotent paid routes), the rolling key-rotation rule and the per-replica check that makes it verifiable, and what "multi-writer safe" deliberately does not mean. |
...
| [release/verification-harness.md](release/verification-harness.md) | Maintainers | `npm run verify:release`: the nineteen steps it runs, how to run one on its own, why a skip is not a pass, why it can never publish, the attribution guard, the upgrade gate, the backup/restore drill, the stale-image defence, the database-free idempotency guard, and what the version and config censuses found the first time they ran. |
...
| [release/v0.5.0-release-notes.md](release/v0.5.0-release-notes.md) | Everyone | **Prepared, not published.** The release candidate for the multi-writer cycle: the theme, the observable changes, the compatibility position, the documents it carries, and ten things it does not claim. |
| [release/v0.5.0-checklist.md](release/v0.5.0-checklist.md) | Maintainers | **Executed.** The gates verified in the multi-writer cycle with their results. |
| [release/v0.6.0-release-notes.md](release/v0.6.0-release-notes.md) | Everyone | **Prepared, not published.** The release candidate for the idempotency cycle: the theme, the retry contract, the failure semantics, the one observable change, the measured cost of the previous cycle's correctness, and ten things it does not claim. |
| [release/v0.6.0-checklist.md](release/v0.6.0-checklist.md) | Maintainers | **Prepared, not executed.** Every gate run in this cycle with its result, the gates specific to idempotency, the published-tag immutability proof, and the publication steps recorded as **not run**. |
