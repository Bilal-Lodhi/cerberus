...
| `DELETE /api/v1/guardian/sessions/:id` | operator key | one identifier | `guardian.delete.complete`, `guardian.delete.partial`, `guardian.delete.failure` | one `delete_session` MCP call | `monitored_sessions`, `micro_events`, `risk_assessments` for that session | `200`, `404`, `500`, `503` | `SESSION_NOT_FOUND`, `PARTIAL_DELETE`, `SESSION_STORE_UNAVAILABLE` | Reports per-component outcomes: `components` names what was removed (`session`, `telemetry`, `assessments`) and `failedComponents` names what was not. A partial deletion is `500 PARTIAL_DELETE` with `retrySafe: true`, and the caches are left alone because the session record is still durable. A store that does not answer is `503` with **nothing attempted**. Retrying is safe: the store removes the derived documents first and the session record last. | — |
...
| `DELETE /guardian/sessions/:id` | `503 SESSION_STORE_UNAVAILABLE`, nothing attempted; `500 PARTIAL_DELETE` when it partly ran, with the components that were removed | unaffected | unaffected |
