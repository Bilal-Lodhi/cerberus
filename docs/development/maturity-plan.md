...
| C | Two processes can safely observe/transition the same session under supported flows | **Met** — both live read surfaces reconcile, the transition boundary is predicate-checked, and `test/integration/multi-process.test.ts` runs **two real API processes against one real MongoDB** across eight flows, including two concurrent terminates and two concurrent ingests |
...
| G | Paid-route duplicate-spend risk reduced or explicitly re-accepted with stronger evidence | **Re-accepted with the exposure measured.** [idempotency-model.md](idempotency-model.md) records the paid calls per route read from the source (`POST /scenarios` spends **twice** — a classifier and then authoring; `POST /auditor/query` also spends **twice** — a pipeline-construction call and then a summarisation call, corrected from an earlier record of one by [paid-operation-state-model.md](paid-operation-state-model.md) §2.3), the three ordinary ways a response is lost, what bounds the blast radius, the claim protocol that would close it, and the exact pending-operation semantics. **Not implemented in this phase**, and stated as such. The `v0.6.0` cycle builds it |
| H | Any idempotency mechanism is durable, race-safe and bounded | **Not applicable** — no mechanism was introduced. The design, its retention bound, and the eight assertions that would prove it are in [idempotency-model.md](idempotency-model.md) §7 |
| I | Per-process rate limiting honestly scoped, or a safe next-step boundary documented | **Met** — [../operations/multi-replica.md](../operations/multi-replica.md) §2.1 states the N-replica multiplier, the decision to keep the limiter local, why a Mongo-backed limiter was rejected (a write on every request's hot path for a backstop), and where per-caller limiting belongs (the proxy, which has the caller identity the baseline does not) |
| J | Shared-key lifecycle risks bounded without inventing accounts | **Met** — the rolling rule ("the new key is everywhere before the old key is anywhere retired"), the per-replica verification loop, the MCP adapter's extra ordering constraint, and an explicit statement that no key generation id or revocation list is being invented, in [../operations/multi-replica.md](../operations/multi-replica.md) §3 |
...
| N | Live-state reconciliation has measured cost, no pathological query amplification | **Partially met.** The query count is bounded by construction and asserted: one `get_session_review` per live-detail request and one `list_sessions` per live-list request regardless of how many sessions are in memory, so there is no N+1 — `session-detail-fallback.test.ts` pins the detail at **exactly one** store call. A **current** latency measurement was taken at release prep (list p50 4.01 ms / p95 5.90 ms; detail p50 1.78 ms / p95 2.42 ms, real MongoDB, 50 sessions, 200 samples). There is **no before/after baseline**, because the pre-change code is no longer in the tree, so the criterion is left partial rather than closed |
