...
| G | Local performance baseline exists | Done — [performance-baseline.md](performance-baseline.md) with `npm run bench`, measured p50/p95/p99 per case. It found two real defects: ingest cost grows linearly with the events a session already holds (29x throughput loss by 5 000 events), and in-memory session state has no per-session cap |
