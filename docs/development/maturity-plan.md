...
| G | Paid-route duplicate-spend risk reduced or explicitly re-accepted with stronger evidence | **Re-accepted with the exposure measured.** [idempotency-model.md](idempotency-model.md) records the paid calls per route read from the source (`POST /scenarios` spends **twice** — a classifier and then authoring; `POST /auditor/query` once), the three ordinary ways a response is lost, what bounds the blast radius, the claim protocol that would close it, and the exact pending-operation semantics. **Not implemented**, and stated as such |
