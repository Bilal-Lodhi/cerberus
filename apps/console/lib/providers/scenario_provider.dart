...
/// Holds the state for threat scenario authoring.
///
/// Retry strategy:
...
class ScenarioProvider extends ChangeNotifier {
  ...
  ScenarioProvider(this._api);

  // ── Public getters ──────────────────────────────────────────────────────────

  ThreatScenarioMatrix? get matrix => _matrix;
  ...
  Future<void> _generateWithAutoRetry(
    ...
    while (_attempt <= _maxAutoRetries) {
      _attempt++;
      try {
        final result = await _api.authorScenario(
          prompt,
  ...
  Future<void> _backoff(int attempt) async {
    ...
    debugPrint(
      '[ScenarioProvider] Auto-retry in ${delay.inMilliseconds}ms (attempt $attempt/$_maxAutoRetries)',
    );
  ...
  String _buildUserMessage(ApiException e) {
    ...
    if (e.statusCode == 504) {
      return 'The AI provider is taking too long to respond. '
