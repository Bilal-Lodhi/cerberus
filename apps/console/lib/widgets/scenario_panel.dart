...
class ScenarioPanel {
  ...
  /// Shows the threat scenario bottom sheet.
  static void show(BuildContext context) {
    Navigator.of(context).push(
      _ScenarioPanelRoute(builder: (ctx) => const _ScenarioPanelContent()),
    );
  }
}
    ...
    return AlertDialog(
      title: Row(
        children: [
          ...
          const Text('Deploy Threat Scenario Matrix'),
        ],
      ),
      content: Form(
        key: _formKey,
...
// ── Custom modal route ────────────────────────────────────────────────────────
class _ScenarioPanelRoute<T> extends PopupRoute<T> {
  _ScenarioPanelRoute({required this.builder});
...
class _ScenarioPanelContent extends StatefulWidget {
...
class _ScenarioPanelContentState extends State<_ScenarioPanelContent> {
  ...
  void _tryClose() {
    final gen = context.read<ScenarioProvider>();
  ...
  void _showDiscardConfirmDialog() {
    ...
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'You have not deployed this threat scenario. '
  ...
  void _resetAndPop() {
    context.read<ScenarioProvider>().reset();
  ...
  void _onGenerate() {
    ...
    _hasDeployed = false;
    context.read<ScenarioProvider>().generate(
      structuredPrompt,
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              Navigator.of(ctx).pop();
              context.read<ScenarioProvider>().cancel();
            },
            style: FilledButton.styleFrom(
              backgroundColor: theme.colorScheme.error,
            ),
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final gen = context.watch<ScenarioProvider>();
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        showResult
                            ? 'Threat Scenario Matrix Generated'
                            : showLoading
                            ? 'Generating...'
                            : showCancelled
              ...
              children: [
                ...
                Expanded(
                  child: FilledButton.icon(
                    ...
                    label: const Text('Author Scenarios'),
                    style: FilledButton.styleFrom(
                      minimumSize: const Size(0, 48),
                      backgroundColor: theme.colorScheme.primary,
                    ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          Text(
            'Generating threat scenario matrix...',
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                const SizedBox(width: 12),
                FilledButton.icon(
                  onPressed: () => context.read<ScenarioProvider>().resume(),
  ...
  Widget _buildErrorState(ThemeData theme, ScenarioProvider gen) {
    return Center(
      child: Padding(
        padding: const EdgeInsets.all(32),
        child: Column(
  ...
  Widget _buildResultState(ThemeData theme, ThreatScenarioMatrix matrix) {
    return ListView(
      padding: const EdgeInsets.fromLTRB(20, 12, 20, 24),
      children: [
        // ── Metadata card ──────────────────────────────────────────────────
              ...
              children: [
                ...
                _metadataRow(
                  ...
                  '${matrix.targetSystems.length} system(s) · '
                      ...
                      '${matrix.penetrationScenarios.length} penetration scenario(s)',
                ),
              ],
            ),
          ),
            ...
            (sys) => Card(
              ...
              child: ListTile(
                leading: Icon(
                  ...
                  size: 20,
                  color: _criticalityColor(sys.criticalityLevel, theme),
                ),
                title: Text(
                  sys.name,
                ...
                subtitle: Text(
                  '${sys.criticalityLevel.toUpperCase()} · '
        ...
        if (matrix.penetrationScenarios.isNotEmpty) ...[
          _buildSheetLabel(theme, 'Penetration Scenarios', Icons.track_changes),
          const SizedBox(height: 8),
          ...matrix.penetrationScenarios.map(
            (scenario) => Card(
              margin: const EdgeInsets.only(bottom: 6),
              elevation: 0,
              color: theme.colorScheme.surfaceContainerLow,
              child: ExpansionTile(
                ...
                subtitle: Text(
                  'Vector: ${scenario.vectorId} · '
                  '${scenario.mandateIds.length} mandate(s)',
                  style: theme.textTheme.labelSmall,
                ),
                children: [
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          'Scoring: ${scenario.scoringFormula.type}',
                          style: theme.textTheme.labelSmall?.copyWith(
                            fontWeight: FontWeight.w600,
                          ),
                        ),
                        const SizedBox(height: 4),
                        Text(
                          'Mandates: ${scenario.mandateIds.join(", ")}',
                          style: theme.textTheme.labelSmall?.copyWith(
                            color: theme.colorScheme.onSurfaceVariant,
                          ),
                        ),
                        if (scenario.exploitCode != null &&
                            scenario.exploitCode!.isNotEmpty) ...[
                          const SizedBox(height: 4),
                          Text(
                            'Exploit Code:',
                            style: theme.textTheme.labelSmall?.copyWith(
  ...
  void _onDeploy(ThreatScenarioMatrix matrix) async {
    ...
    // The matrix identifier is the persisted scenario-matrix ID.
    final matrixId = matrix.metadata.matrixId;
    ...
    if (matrix.targetSystems.isNotEmpty) {
      ...
      targetSystemValue = first.systemId.isNotEmpty
          ? first.systemId
          : first.name;
      targetSystemLabel = first.name.isNotEmpty
          ? first.name
          : targetSystemValue;
    } else {
      ...
      if (session.sessionId.isNotEmpty) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Threat scenario matrix deployed — Session ${session.sessionId}',
              ),
  ...
  Widget _buildVectorCard(ThemeData theme, ThreatVector vector) {
    final severityColor = vector.severity == 'critical'
        ? Colors.red
        : vector.severity == 'high'
        ? Colors.orange
        : Colors.green;
    final vectorTypeIcon = vector.vectorType == 'token_injection'
        ? Icons.code
        : vector.vectorType == 'data_exfiltration'
        ? Icons.upload_file
        : vector.vectorType == 'transfer_interception'
        ? Icons.swap_horiz
        : vector.vectorType == 'privilege_escalation'
        ? Icons.vpn_key
        : Icons.bug_report;
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Row(
          children: [
            ...
            const SizedBox(width: 8),
            Text(
              '| Investigation: ${vector.investigationTimeMinutes}m',
              style: theme.textTheme.labelSmall?.copyWith(
                color: theme.colorScheme.outline,
              ),
            ),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // Workspace seed
                if (vector.workspaceSeed != null &&
                    vector.workspaceSeed!.isNotEmpty) ...[
                  const SizedBox(height: 10),
                  Text(
                    'Workspace Seed',
                    style: theme.textTheme.labelSmall?.copyWith(
                ...
                Row(
                  children: [
                    ...
                    Text(
                      'Investigation time: ${vector.investigationTimeMinutes}m',
                      style: theme.textTheme.labelSmall?.copyWith(
                        color: theme.colorScheme.outline,
                      ),
                    ),
                    ...
                    const SizedBox(width: 4),
                    Text(
                      'Risk score: ${vector.riskScore.toStringAsFixed(0)}',
                      style: theme.textTheme.labelSmall?.copyWith(
                        color: theme.colorScheme.outline,
                      ),
                    ),
                ...
                if (vector.expectedRemediation != null &&
                    vector.expectedRemediation!.isNotEmpty) ...[
                  const SizedBox(height: 8),
                  Text(
                    'Expected remediation: ${vector.expectedRemediation}',
                    style: theme.textTheme.labelSmall?.copyWith(
                ...
                // Detection rules
                if (vector.detectionRules.isNotEmpty) ...[
                  const SizedBox(height: 10),
                  Text(
                    'Detection Rules (${vector.detectionRules.length})',
                    style: theme.textTheme.labelSmall?.copyWith(
                      fontWeight: FontWeight.w600,
                    ),
                  ),
                  const SizedBox(height: 4),
                  ...vector.detectionRules.map(
                    (rule) => Padding(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (rule.signalPattern.isNotEmpty) ...[
                              const SizedBox(height: 4),
                              Text(
                                'Signal pattern: ${rule.signalPattern}',
                                style: theme.textTheme.labelSmall?.copyWith(
                                  color: theme.colorScheme.onSurfaceVariant,
                                ),
                              ),
                            ],
                            if (rule.expectedSignal.isNotEmpty) ...[
                              const SizedBox(height: 2),
                              Text(
                                'Expected signal: ${rule.expectedSignal}',
                                style: theme.textTheme.labelSmall?.copyWith(
                                  color: Colors.green,
                                ),
                              ),
                            ],
                            if (rule.evaluationWindowMs > 0) ...[
                              const SizedBox(height: 2),
                              Text(
                                'Evaluation window: ${rule.evaluationWindowMs}ms',
                                style: theme.textTheme.labelSmall?.copyWith(
                                  fontStyle: FontStyle.italic,
                                  color: theme.colorScheme.outline,
                                ),
                ...
                // Mitigation options
                if (vector.mitigations.isNotEmpty) ...[
                  const SizedBox(height: 10),
                  Text(
                    'Mitigation Options (${vector.mitigations.length})',
                    style: theme.textTheme.labelSmall?.copyWith(
                      fontWeight: FontWeight.w600,
                    ),
                  ),
                  const SizedBox(height: 4),
                  ...vector.mitigations.map(
                    (option) => Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(
                                  option.text,
                                  style: theme.textTheme.labelSmall?.copyWith(
                                    fontWeight: option.isRecommended
                                        ? FontWeight.w600
                                        : FontWeight.normal,
                                    color: option.isRecommended
